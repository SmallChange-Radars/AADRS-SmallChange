...
export class RegisterFormComponent implements OnInit {
  ...
  passwordErrorTextmsg: string =
    'Invalid Password - Must contain between 6 and 18 letters, numbers, underscores or hyphens.';
  ...
  onSubmit() {
    if (this.registerForm.valid) {
      this.identity.type =
        this.registerFormControl.country.value.toLowerCase() === 'us'
          ? 'SSN'
          : 'Passport';
      ...
      let client: Client = new Client(
        ...
        registerFormValue.dob,
        registerFormValue.country,
        registerFormValue.pincode,
        [this.identity],
      ...
      this.service.pushUser(client).subscribe({
        next: (data) => {
          console.log('Posted');
          alert('Registered Successfully! Please fill Account Details');
